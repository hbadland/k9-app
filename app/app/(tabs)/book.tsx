...
export default function Book() {
  ...
  // Load all slot dates for the visible month (for dots)
  useEffect(() => {
    const from = `${year}-${String(month + 1).padStart(2, '0')}-01`;
    const last = new Date(year, month + 1, 0).getDate();
    const to   = `${year}-${String(month + 1).padStart(2, '0')}-${String(last).padStart(2, '0')}`;
    api.get(`/availability?from=${from}&to=${to}`)
  ...
  const firstMon = firstDay === 0 ? 6 : firstDay - 1; // offset so Mon=0
  ...
  return (
    <View style={s.root}>
      ...
      <View style={s.header}>
        ...
        {/* Calendar — only shown on Book tab */}
        {tab === 'book' && <View style={s.cal}>
      ...
      {/* ── My Bookings ── */}
      {tab === 'mine' && (
        <ScrollView style={s.slotsScroll} showsVerticalScrollIndicator={false}
          contentContainerStyle={{ paddingBottom: 120, paddingTop: 16 }}>
          {!myBookingsLoaded
            ? <ActivityIndicator color={C.gold} style={{ marginTop: 32 }} />
            : myBookings.length === 0
              ? <Text style={s.slotsEmpty}>No bookings yet.</Text>
              : myBookings.map(b => {
      ...
      {/* ── Slots ── */}
      {tab === 'book' && <ScrollView style={s.slotsScroll} showsVerticalScrollIndicator={false}
        ...
        {selectedDate ? (
          <>
            ...
            {loading
              ...
              : slots.length === 0
                ? <Text style={s.slotsEmpty}>No slots available on this date.</Text>
                : slots.map((slot) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={slot.id}
                        style={[s.slot, selected && s.slotSelected, full && s.slotFull]}
                        onPress={() => !full && setSelectedSlot(slot)}
                        disabled={full}
                      >
                        <View style={[s.slotTime, selected && s.slotTimeSelected]}>
                        ...
                        <View style={s.slotInfo}>
                          ...
                          <Text style={s.slotDetail}>
                            {fmtPrice(slot.price_pence)} · {slot.capacity === 480 ? 'Full day' : `${slot.capacity * 60}min`}
                          </Text>
                        </View>
            ...
            {/* Dog selector */}
            {dogs.length > 1 && selectedSlot && (
              <View style={s.dogSelect}>
                <Text style={s.dogSelectLabel}>Booking for</Text>
          ...
          <Text style={s.slotsEmpty}>Select a date to see available slots.</Text>
        )}
      </ScrollView>}

      {/* ── CTA ── */}
      {tab === 'book' && selectedSlot && (
        <View style={s.ctaWrap}>
          <TouchableOpacity onPress={confirmBook} disabled={booking} style={s.ctaBtn}>
            <LinearGradient
              colors={confirmed ? ['#4CAF7A', '#6dd89a'] : [C.gold, C.goldLight]}
              style={s.ctaGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
              {booking
                ? <ActivityIndicator color={C.dark} />
                : <Text style={s.ctaText}>
                    {confirmed ? '✓ Booking requested!' : `Confirm — ${selectedSlot.service_name} · ${fmt12(selectedSlot.start_time).h}:${fmt12(selectedSlot.start_time).m} ${fmt12(selectedSlot.start_time).ampm}`}
                  </Text>
              }
            </LinearGradient>
          </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  header:         { backgroundColor: C.dark2, borderBottomWidth: 1,
                    borderBottomColor: 'rgba(255,255,255,0.05)', paddingTop: 64, paddingHorizontal: 20, paddingBottom: 16 },
  ...
  // Slots
  slotsScroll:    { flex: 1, paddingHorizontal: 20, paddingTop: 16 },
  slotsDayTitle:  { fontSize: 11, textTransform: 'uppercase', letterSpacing: 1.5,
                    color: C.textDim, marginBottom: 12 },
  slotsEmpty:     { color: C.muted, fontSize: 14, textAlign: 'center', marginTop: 32 },
  slot:           { backgroundColor: C.dark3, borderWidth: 1, borderColor: C.dark4,
                    borderRadius: 18, padding: 15, marginBottom: 10,
                    flexDirection: 'row', alignItems: 'center', gap: 14 },
  slotSelected:   { borderColor: `${C.gold}73`, backgroundColor: `${C.gold}0D` },
  ...
  slotTimeSelected:{ backgroundColor: `${C.gold}24` },
  ...
  // Dog selector
  dogSelect:      { marginTop: 16 },
  dogSelectLabel: { fontSize: 11, color: C.textDim, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8 },
  ...
  ctaWrap:        { position: 'absolute', bottom: 0, left: 0, right: 0,
                    padding: 16, paddingBottom: 32, backgroundColor: C.dark2,
                    borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.06)' },
  ctaBtn:         { borderRadius: 18, overflow: 'hidden' },
  ctaGrad:        { padding: 17, alignItems: 'center' },
  ctaText:        { color: C.dark, fontWeight: '700', fontSize: 15 },
});
